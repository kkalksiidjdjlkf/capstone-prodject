...
class Reranker:
    ...
    def __init__(self, config: Dict):
        ...
        """
        self.config = config
        configured = self.config.get("model", {}).get("device_rerank")
        self.device = configured or _auto_device()
    ...
    def _initialize_model(self) -> None:
        ...
        try:
            ...
            # Move to device (should be safe now since loaded directly)
            self.model.to(self.device)
            self.model.eval()

            self.logger.info(f"Initialized reranker model: {self.model_name} on {self.device}")
