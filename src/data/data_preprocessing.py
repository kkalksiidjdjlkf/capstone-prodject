...
class CustomSemanticChunker:
    """
    Custom semantic chunker using embeddings to split text at semantic boundaries.
    """
    def __init__(self, embedding_model, threshold=0.7, min_chunk_size=100, max_chunk_size=1000):
        self.embedding_model = embedding_model
        self.threshold = threshold  # Similarity threshold for splitting
        self.min_chunk_size = min_chunk_size
        self.max_chunk_size = max_chunk_size
    
    def split_text(self, text: str, metadata: Dict = None) -> List[Document]:
        """
        Split text semantically based on embedding similarities.
        """
        # Split into sentences (simple approach; can use spaCy for better)
        ...
        for i in range(1, len(sentences)):
            # Compute similarity with current chunk's average embedding
            similarity = cosine_similarity([current_embedding], [embeddings[i]])[0][0]
            
            # If similarity is low, start new chunk
            if similarity < self.threshold or len(' '.join(current_chunk + [sentences[i]])) > self.max_chunk_size:
                # Finalize current chunk
                chunk_text = ' '.join(current_chunk)
                if len(chunk_text) >= self.min_chunk_size:
                    chunks.append(Document(page_content=chunk_text, metadata=metadata or {}))
                current_chunk = [sentences[i]]
                current_embedding = embeddings[i]
            else:
                # Add to current chunk and update average embedding
...
class DataPreprocessor:
    ...
    def __init__(self, config: Dict, embedding_model=None):
        """
    ...
    def process_documents(self, doc_contents: Dict[str, str]) -> List[Document]:
        ...
        all_chunks = []
        for doc_path, doc_list in tqdm(doc_contents.items(), desc="Chunking documents"):
            result = self.process_single_document((doc_path, doc_list))
            if isinstance(result, tuple) and len(result) == 2:
                _, chunks = result
                all_chunks.extend(chunks)
            else:
                self.logger.error(f"Invalid result from process_single_document: {result}")
    ...
    def process_single_document(self, args: Tuple[str, str]) -> Tuple[str, List[Document]]:
        ...
        # Check cache with unique key for preprocessing (to avoid conflicts with ingestion cache)
        # TEMPORARILY DISABLE CACHING FOR TESTING
        # cache_key = f"preprocessed_{doc_path}"
        ...
        if "is_markdown" in docs[0].metadata.keys() and docs[0].metadata["is_markdown"]:
           ...
           chunks = self.generate_csv_chunks(docs)
            ...
            # Default text splitting - use semantic chunking if embedding_model available
            if self.embedding_model:
                semantic_splitter = CustomSemanticChunker(self.embedding_model)
                chunks = semantic_splitter.split_text(content, {"source": doc_path})
            else:
                text_splitter = RecursiveCharacterTextSplitter(
                    chunk_size=self.chunk_size,
                    chunk_overlap=self.chunk_overlap,
        ...
        # self.cache_manager.set(cache_key, result)  # Temporarily disabled for testing
        return result
