...
def setup_logging(config: Dict) -> None:
    ...
    """
    # Performance-related environment variables (safe defaults)
    # These need to be set early (before heavy CPU ops / model init) to take effect.
    omp_threads = config.get("processing", {}).get("OMP_NUM_THREADS")
    if omp_threads:
